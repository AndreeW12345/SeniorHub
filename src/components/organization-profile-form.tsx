...
export type OrganizationProfileFormProps = {
  /** Firestore document id — never editable in the form. */
...
export function OrganizationProfileForm({
  ...
  useEffect(() => {
    ...
    async function loadOrganization() {
      ...
      try {
        const organization = await fetchOrganizationByIdFromFirestore(trimmedOrganizationId);
        if (!isMounted) {
          return;
        }
        ...
        setPublicSlug(organization.slug);
      } finally {
        if (isMounted) {
          setIsLoading(false);
        }
  ...
  const handleSave = async () => {
    ...
    if (!trimmedOrganizationId) {
      setSubmitError('Organisationen kunde inte hittas.');
      return;
    }
    ...
    if (localLogoUri) {
      ...
      const uploadResult = await uploadOrganizationLogo(localLogoUri, trimmedOrganizationId);
    ...
    const result = await saveOrganizationToFirestore(trimmedOrganizationId, {
      name,
      description,
      website,
      membershipUrl,
    ...
    setPublicSlug(result.organization.slug);
  ...
  if (isLoading) {
    return (
      <ScreenLayout title={title} subtitle="Hämtar organisation" showBackButton>
  ...
  if (notFound) {
    return (
      <ScreenLayout title={title} subtitle="Organisationen hittades inte" showBackButton>
        <ThemedText type="bodyLarge" themeColor="textSecondary" style={styles.notFoundText}>
  ...
  return (
    <ScreenLayout
      ...
      <View style={styles.form}>
        <AdminFormSection
          title="Organisation"
          description="Organisations-id är låst. Publik slug uppdateras från namnet när du sparar.">
          <FormField
